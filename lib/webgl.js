...
function drawDrawable(drawable, blend, diff, ambient){
    gl.uniformMatrix4fv(uniform_locs["modelmat"], false, drawable["modelmatrix"]);
    for (let p = 0; p < drawable["vertexbuffer"].length; p++) {
        connectBufferToAttribute(gl, gl.ARRAY_BUFFER, drawable["vertexbuffer"][p]["vertices"], attrib_locs["vertexpos"], 3);
        connectBufferToAttribute(gl, gl.ARRAY_BUFFER, drawable["normalbuffer"][p], attrib_locs["vertexnorm"], 3);
        connectBufferToAttribute(gl, gl.ARRAY_BUFFER, drawable["texcoordbuffer"][p], attrib_locs["texcoord"], 2);
        ...
        if(ambient == undefined)
            ...
            gl.uniform3fv(uniform_locs["Ka"], ambient);

        if(blend == undefined)
            gl.uniform1f(uniform_locs["blend"], 1.0);
        else
            gl.uniform1f(uniform_locs["blend"], blend);

        if(diff == undefined)
            gl.uniform1i(uniform_locs["diff"], 1);
        else
            gl.uniform1i(uniform_locs["diff"], diff);
        
        gl.drawArrays(gl.TRIANGLES, 0, drawable["vertexbuffer"][p]["n_vertices"]);
    }
}

function drawLine(line) {
    gl.uniform3fv(uniform_locs["Ka"], [1.0, 1.0, 1.0]);
    gl.uniform1i(uniform_locs["diff"], 0);
    gl.uniform1f(uniform_locs["blend"], 1.0);
    gl.uniformMatrix4fv(uniform_locs["modelmat"], false, identMat4f());
    gl.bindTexture(gl.TEXTURE_2D, line["tex"]);
    connectBufferToAttribute(gl, gl.ARRAY_BUFFER, line["vertexbuffer"], attrib_locs["vertexpos"], 3);
