...
function separatingAxisTest(drawable1, drawable2) {
  ...
  // This assumes the smallest axis needs to be flipped depending on the relative centroid positions
  ...
  if (dotVec3f(direction, smallestAxis) < 0) {
    smallestAxis = smallestAxis.map(value => -value); // Invert the axis
  }

  let directionOfCollision = smallestAxis.map(value => value * minOverlap);
  //directionOfCollision = multScalVec3f(0.0, directionOfCollision);

  return {
    collision: true,
    directionOfCollision: [centroid1, directionOfCollision]
  };
}
