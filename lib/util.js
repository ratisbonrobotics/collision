...
async function makeConvexDrawable(drawable){
  ...
  let obj_path = '/data/' + drawable["keys"][0] + '_convex_hull.obj';
  let obj = await parseOBJ(obj_path);
  ...
  for (const [index, [key, value]] of entries.entries()) {
    convex_drawable["vertexbuffer"][index] = {};
    convex_drawable["vertexbuffer"][index]["vertices"] = createBuffer(gl, gl.ARRAY_BUFFER, value["v"]);
    convex_drawable["vertexbuffer"][index]["n_vertices"] = Math.floor(value["v"].length / 3);
    convex_drawable["texcoordbuffer"][index] = createBuffer(gl, gl.ARRAY_BUFFER, value["vt"]);
    convex_drawable["normalbuffer"][index] = createBuffer(gl, gl.ARRAY_BUFFER, value["vn"]);
    convex_drawable["texture"][index] = textures[index];

    convex_drawable["vertices"][index] = value["v"];
    convex_drawable["keys"][index] = key;

    if(value["m"][0]){
      convex_drawable["material"][index] = {
        "Ka": value["m"][0]["Ka"] || [1, 1, 1],
        "Kd": value["m"][0]["Kd"] || [1, 1, 1],
        "Ke": value["m"][0]["Ke"] || [1, 1, 1],
        "Ks": value["m"][0]["Ks"] || [1, 1, 1],
