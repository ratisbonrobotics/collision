...
function separatingAxisTest(drawable1, drawable2) {
  const axes1 = getUniqueAxes(drawable1);
  const axes2 = getUniqueAxes(drawable2);
  const axes = [...axes1, ...axes2];

  for (const axis of axes) {
    const projection1 = projectOntoAxis(drawable1, axis);
    const projection2 = projectOntoAxis(drawable2, axis);
...
function getUniqueAxes(drawable) {
  const axes = new Set();
  const transformedVertices = drawable.vertices.map(vertex => multMat4fVec3f(drawable.modelmatrix, [...vertex, 1]).slice(0, 3));

  // Generate edge axes
  for (let i = 0; i < transformedVertices.length; i++) {
    const v1 = transformedVertices[i];
    for (let j = i + 1; j < transformedVertices.length; j++) {
      const v2 = transformedVertices[j];
      const edge = subVec3f(v2, v1);
      const axis = normVec3f(edge);
      axes.add(axis.join(','));
    }
  }
  ...
  for (let i = 0; i < transformedVertices.length; i++) {
    ...
    for (let j = i + 1; j < transformedVertices.length; j++) {
      const v2 = transformedVertices[j];
      for (let k = j + 1; k < transformedVertices.length; k++) {
        const v3 = transformedVertices[k];
  ...
  return Array.from(axes).map(str => str.split(',').map(parseFloat));
}

function projectOntoAxis(drawable, axis) {
  const transformedVertices = drawable.vertices.map(vertex => multMat4fVec3f(drawable.modelmatrix, [...vertex, 1]).slice(0, 3));
  let min = Infinity;
  let max = -Infinity;
...
function multMat4fVec3f(matrix, vector) {
  ...
  const resultX = m00 * x + m01 * y + m02 * z + m03 * w;
  const resultY = m10 * x + m11 * y + m12 * z + m13 * w;
  const resultZ = m20 * x + m21 * y + m22 * z + m23 * w;
  const resultW = m30 * x + m31 * y + m32 * z + m33 * w;
