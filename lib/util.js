...
async function makeConvexDrawable(drawable){
  ...
  let obj = await parseOBJ('/data/' + drawable["keys"][0] + '_convex_hull.obj');

  const [key, value] = Object.entries(obj)[0];
  convex_drawable["vertexbuffer"][0] = {};
  convex_drawable["vertexbuffer"][0]["vertices"] = createBuffer(gl, gl.ARRAY_BUFFER, value["v"]);
  convex_drawable["vertexbuffer"][0]["n_vertices"] = Math.floor(value["v"].length / 3);
  convex_drawable["texcoordbuffer"][0] = createBuffer(gl, gl.ARRAY_BUFFER, value["vt"]);
  convex_drawable["normalbuffer"][0] = createBuffer(gl, gl.ARRAY_BUFFER, value["vn"]);
  convex_drawable["texture"][0] = await createTexture(gl, createColorImageURL([1, 1, 1])); // WHITE
  convex_drawable["vertices"][0] = value["v"];
  convex_drawable["keys"][0] = key;
