...
function separatingAxisTest(drawable1, drawable2) {
  ...
  for (const axis of axes) {
      const projection1 = projectVertices(drawable1, axis);
      const projection2 = projectVertices(drawable2, axis);
...
function getUniqueAxes(drawable) {
  ...
  for (let i = 0; i < drawable.vertices[0].length; i += 9) {
      const v1 = drawable.vertices[0].slice(i, i + 3);
      const v2 = drawable.vertices[0].slice(i + 3, i + 6);
      const v3 = drawable.vertices[0].slice(i + 6, i + 9);
...
function projectVertices(drawable, axis) {
  let min = Infinity;
  let max = -Infinity;

  for (let i = 0; i < drawable.vertices[0].length; i += 3) {
      const vertex = drawable.vertices[0].slice(i, i + 3);
