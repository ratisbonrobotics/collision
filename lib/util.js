...
function separatingAxisTest(drawable1, drawable2) {
  // Transform vertices according to their model matrices
  console.log(drawable1);
  ...
  // Retrieve all unique axes to test from both drawables
  const axes = getUniqueAxes(vertices1).concat(getUniqueAxes(vertices2));
  ...
  for (let i = 0; i < axes.length; i++) {
      ...
      if (!projectionsOverlap(projection1, projection2)) {
          // If they do not overlap on this axis, no collision
          return false;
      }
  }

  // If all axes projections overlap, there is a collision
  return true;
}
