...
async function makeConvexDrawable(drawable){
  ...
  let convex_vertices = await calculateConvexHull(extractUniqueVertices(drawable["vertices"][0]), drawable["keys"][0]);
...
function calcAabb(vertices) {
  if (vertices.length % 3 !== 0) {
    throw new Error("The length of the vertices array should be a multiple of 3.");
  }
...
async function calculateConvexHull(vertices){
  return calcAabb(vertices.flat()).flat();
}
