...
function separatingAxisTest(drawable1, drawable2) {
  ...
  let mtvAxis = null;
  let deepestVertex1 = null;
  let deepestVertex2 = null;
  ...
  for (const axis of axes) {
    ...
    if (!overlap(projection1, projection2)) {
      ...
      // Calculate overlap
      const overlap = Math.min(projection1[1] - projection2[0], projection2[1] - projection1[0]);
      if (overlap < minOverlap) {
        minOverlap = overlap;
        mtvAxis = axis;

        // Identify the vertex of deepest penetration for drawable1 and drawable2
        deepestVertex1 = findDeepestVertex(transformedVertices1, axis, projection1, true);
        deepestVertex2 = findDeepestVertex(transformedVertices2, axis, projection2, false);
      }
    }
  }
  ...
  return {
    ...
    deepestVertex1: deepestVertex1,
    deepestVertex2: deepestVertex2
  };
}

function findDeepestVertex(vertices, axis, projection, isMin) {
  let extremeValue = isMin ? Infinity : -Infinity;
  let deepestVertex = null;

  for (let i = 0; i < vertices.length; i += 3) {
    const vertex = vertices.slice(i, i + 3);
    const projectionValue = dotVec3f(vertex, axis);

    if ((isMin && projectionValue < extremeValue) || (!isMin && projectionValue > extremeValue)) {
      extremeValue = projectionValue;
