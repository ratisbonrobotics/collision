...
async function makeConvexDrawable(drawable){
  ...
  const arrayOfVertices = Array.from({ length: drawable["vertices"][0].length / 3 }, (_, i) => drawable["vertices"][0].slice(i * 3, i * 3 + 3));
  const arrayOfFaces = Array.from({ length: arrayOfVertices.length / 3 }, (_, i) => arrayOfVertices.slice(i * 3, i * 3 + 3));
  let convex_vertices = await calculateConvexHull(arrayOfFaces, drawable["keys"][0]);
...
async function calculateConvexHull(arrayOfFaces, name){
...
function extractUniqueVertices(vertices){
  return [...new Set(Array.from({ length: vertices.length / 3 }, (_, i) => vertices.slice(i * 3, i * 3 + 3)).map(JSON.stringify))].map(JSON.parse);
}